...
def get_client(provider: str) -> OpenAI:
    if provider == "ollama":
        return OpenAI(base_url=OLLAMA_BASE_URL, api_key="ollama",timeout=LLM_TIMEOUT*7)
    return OpenAI(api_key=os.getenv("OPENAI_API_KEY"),timeout=LLM_TIMEOUT)


def run_tool_call(tool_call, raw_text: str):
    args = json.loads(tool_call.function.arguments)

    code = sanitize_code_for_sandbox(args.get("code_string", ""))
    result = run_python_parser(code, raw_text, timeout=20)  # use trusted raw_text
...
def process_data(raw_text, requirements, provider, model_name):
    ...
    attempts = 0
    last_extracted = None
    last_code_attempt = "(not available)"
    ...
    while True:
        ...
        for call in msg.tool_calls:
            attempts += 1
            tool_msg = run_tool_call(call, raw_text)
            messages.append(tool_msg)
            payload = json.loads(tool_msg["content"])
            last_code_attempt = payload.get("executed_code", last_code_attempt)
        ...
        if attempts >= 5:
            ...
            yield "Retry limit reached (best effort).", j, last_code_attempt
            return
